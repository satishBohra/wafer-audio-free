if (window.location.href.indexOf('?page=wafer-audio-free') > -1) {
...
/* WAFER AUDIO FREE CUSTOM JS */
var fetchWaferAudioFree = function() {
    jQuery.post('/wp-admin/admin-ajax.php', {
        action: 'wafer_audio_free_fetch',
        nonce: document.querySelector('#_wpnonce').value
    }, function(response) {
        wafData = JSON.parse(response);
    });
}

var saveWaferAudioFree = function(obj) {
    // run saving animation
    document.querySelector('.save').classList.add('active');
    document.querySelector('.save').innerHTML = '<em>Saving...</em>';

    jQuery.post('/wp-admin/admin-ajax.php', {
        action: 'wafer_audio_free_save',
        wafer_audio_free_obj: JSON.stringify(obj),
        nonce: document.querySelector('#_wpnonce').value
    }, function(response) {
        // end saving animation
        fetchWaferAudioFree();
        setTimeout(function() {
...
var updateWafDataObj = function() {
    var wafPlayers = document.querySelectorAll('.wafer-audio-free-player');
    var newWafData = {};
    var playerIndex = 0;
    wafPlayers.forEach((player, index) => {
        currentPlayer = document.querySelectorAll('.wafer-audio-free-player')[index];
        let playerInputs = currentPlayer.querySelectorAll('input');
        playerIndex = (index + 1);
        newWafData[ 'player' + playerIndex ] = {
            playerTitle: playerInputs[0].value,
            selected: playerInputs[1].checked,
        }
        var track = {};
        playerInputs.forEach((input, index) => {
            if (!input.classList.contains('wafer-audio-free-player-title') && !input.classList.contains('wafer-audio-free-select-player')) {
                var trackNumber = Math.floor((index - 1)/3);
                ...
                if (index > 1 && (index % 3) == 0) {
                    // this is the artist
                    track.artist = input.value;
                }
                if (index > 1 && (index % 3) == 1) {
                    // this is the url
                    track.url = input.value;
                    newWafData['player' + playerIndex]['track' + trackNumber] = track;
                    track = {};
                }
            }
        });
    ...
    wafData = newWafData;
    // setOption('wafer_audio_free_obj', JSON.stringify(newWafData));
    saveWaferAudioFree(newWafData);
}

document.querySelector('.wafer-audio-free-btn.saving').addEventListener('click', updateWafDataObj);

function wafMediaButton(t) {

    // setting button index, which was clicked?

    var buttonText = t.dataset.buttonIndex;
    ...
    frame.on("select", function () {
      var selectionCollection = frame.state().get("selection"); // for multiple select

      var selectedFirst = frame.state().get("selection").first().toJSON();

      t.parentNode.querySelector('.wafer-audio-free-track-source').value = selectedFirst.url;

      updateWafDataObj();

      // for multiple select: console.log(selectionCollection.toJSON());
    });
...
var waferTrackHTML = `<div class='wafer-track'><span class="dashicons dashicons-move"></span><input type='text' class='wafer-audio-free-track-title' placeholder='track title' /><input type='text' class='wafer-audio-free-track-artist' placeholder='artist' /><input type='text' class='wafer-audio-free-track-source' placeholder='audio url' /><button class='open-select-frame' onclick='wafMediaButton(this)'><span class="dashicons dashicons-media-audio"></span></button><button class='wafer-audio-free-btn' onclick='wafDeleteTrack(this);'><span class="dashicons dashicons-trash"></span></button></div>`;


var newWafPlayer = function() {
    var wafObjKeyCount = Object.keys(wafData).length;
    if (wafObjKeyCount < 1) {
        totalPlayers = document.querySelector('.wafer-audio-free-players').children.length;

        var playerCont = document.createElement('div');

        playerCont.id = 'wafPlayer' + (totalPlayers + 1);

        playerCont.classList.add('wafer-audio-free-player');

        var playerInner = `<div class='wafer-audio-free-player-toolbar'><input type='text' class='wafer-audio-free-player-title' placeholder='player title' /><input type='checkbox' class='wafer-audio-free-select-player' /><button onclick='wafAddTrack(this);' class='wafer-audio-free-btn' title='Add Track'><span class="dashicons dashicons-plus"></span></button><span class='wafer-audio-free-shortcode'></span></div>` + waferTrackHTML;

        playerCont.innerHTML = playerInner;

        playerCont.querySelectorAll('input').forEach((element, index) => {
            element.addEventListener('input', updateWafDataObj);
        });

        document.querySelector('.wafer-audio-free-players').appendChild(playerCont);

        new Sortable(playerCont, {
            handle: '.dashicons-move', // handle's class
            draggable: '.wafer-track', // elements which can be drug
            animation: 150,
        ...
        var wantsToBuyPro = confirm('Sorry! You can only create one player with the free version. Would you like to buy the pro version?');
        if (wantsToBuyPro == true) {
            window.open('https://www.wafer-audio.com');
        }
    }

}

document.querySelectorAll('.wafer-audio-free-player').forEach((playerCont, index) => {
    new Sortable(playerCont, {
        handle: '.dashicons-move', // handle's class
        draggable: '.wafer-track', // elements which can be drug
        animation: 150,
...
document.querySelector('.wafer-audio-free-new-player').addEventListener('click', newWafPlayer);

// add event listeners to all inputs on load
var inputsInit = function() {
    document.querySelectorAll('.wafer-audio-free-players input').forEach((element, index) => {
        element.addEventListener('input', updateWafDataObj);
    });
}
...
var wafDeletePlayers = function() {
    var selectedPlayers = document.querySelectorAll('.wafer-audio-free-select-player:checked');
    if (selectedPlayers.length > 0) {
        var areYouSure = confirm('Are you sure you want to delete the selected player(s)?');
        if (areYouSure === true) {
            selectedPlayers.forEach(function(element, index) {
                element.parentNode.parentNode.parentNode.removeChild(element.parentNode.parentNode);
            });
            updateWafDataObj();
        }
    } else {
        alert('You must select a player to delete...');
    }
}

var wafAddTrack = function(t) {
    // wafer track parent element
    waferTrack = document.createElement('div');
    waferTrack.classList.add('wafer-track');
    // drag element
    var dragTarget = document.createElement('span');
    dragTarget.classList.add('dashicons', 'dashicons-move');
    // text input track title
    var trackTitle = document.createElement('input');
    trackTitle.type = 'text';
    trackTitle.classList.add('wafer-audio-free-track-title');
    trackTitle.placeholder = 'track title';
    // text input track artist 
    var trackArtist = document.createElement('input');
    trackArtist.type = 'text';
    trackArtist.classList.add('wafer-audio-free-track-artist');
    trackArtist.placeholder = 'artist';
    // text input track source
    var trackSource = document.createElement('input');
    trackSource.type = 'text';
    trackSource.classList.add('wafer-audio-free-track-source');
    trackSource.placeholder = 'audio url';
    // media button
    var mediaButton = document.createElement('button');
    mediaButton.classList.add('open-select-frame');
    mediaButton.setAttribute('onclick', 'wafMediaButton(this)');
    var mediaIcon = document.createElement('span');
    mediaIcon.classList.add('dashicons', 'dashicons-media-audio');
    mediaButton.appendChild(mediaIcon);
    // delete track button 
    var deleteTrack = document.createElement('button');
    deleteTrack.classList.add('wafer-audio-free-btn');
    deleteTrack.setAttribute('onclick', 'wafDeleteTrack(this)');
    var trashIcon = document.createElement('span');
    trashIcon.classList.add('dashicons', 'dashicons-trash');
    deleteTrack.appendChild(trashIcon);
    // appending children
    waferTrack.appendChild(dragTarget);
    waferTrack.appendChild(trackTitle);
    waferTrack.appendChild(trackArtist);
    waferTrack.appendChild(trackSource);
    waferTrack.appendChild(mediaButton);
    waferTrack.appendChild(deleteTrack);
    waferTrack.querySelectorAll('input').forEach((element, index) => {
        element.addEventListener('input', updateWafDataObj);
    });
    t.parentNode.parentNode.appendChild(waferTrack);
    updateWafDataObj();
}

var wafDeleteTrack = function(t) {
    t.parentNode.parentNode.removeChild(t.parentNode);
    updateWafDataObj();
}

} // end "is wafer audio free admin page" condition